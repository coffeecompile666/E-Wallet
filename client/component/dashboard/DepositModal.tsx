...
import { deposit, getTransferByID, getWalletMe } from '@/api/wallet';
...
export default function DepositModal({ open, onClose, bankAccounts, onSuccess, onOpenLinkBank }: DepositModalProps) {
      ...
      try {
        ...
        if (status === 'COMPLETED' || status === 'FAILED') {
          ...
          if (status === 'COMPLETED') {
            ...
            addAlert(<>Nạp tiền thất bại từ phía ngân hàng.</>);
          }
          onClose();
        }
      } catch (err) {
  ...
  const handleFormSubmit = () => {
    ...
    if (!selectedBankAccountId) {
      addAlert(<>Vui lòng liên kết và chọn tài khoản ngân hàng để nạp tiền</>);
      return;
    }
  ...
  const handleDepositExecute = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    ...
    try {
      ...
      if (res?.data) {
        ...
        addAlert(<>Yêu cầu nạp tiền đã được gửi tới ngân hàng. Vui lòng đợi...</>);
      }
    } catch (err: any) {
      if (err.code === 1047) {
        ...
        setError(err.userMessage || 'Nạp tiền thất bại. Vui lòng kiểm tra lại mã PIN.');
      }
    } finally {
  ...
  return (
    <AppDialog open={open} onClose={() => modalStep !== 'polling' && onClose()} title="Nạp tiền vào ví">
      {modalStep === 'polling' && (
        <VerifyingState>
          <VerifyingIconWrapper>
            <CreditCard size={28} />
          </VerifyingIconWrapper>
          <h4>Đang xử lý nạp tiền</h4>
          <p>Ngân hàng đang xử lý giao dịch nạp tiền của bạn. Vui lòng không đóng cửa sổ này...</p>
        </VerifyingState>
      )}

      {modalStep === 'pin' && (
        <Form onSubmit={handleDepositExecute}>
          ...
          <p style={{ fontSize: 'var(--font-sm)', color: 'var(--text-secondary)', textAlign: 'center', margin: '0 0 var(--space-2) 0' }}>
            Vui lòng nhập mã PIN giao dịch 6 chữ số để xác thực nạp {formatVND(parseFloat(depositAmount))}.
          </p>
          ...
          <Footer>
            <Button variant="ghost" type="button" onClick={onClose} disabled={isDepositLoading}>Hủy bỏ</Button>
            <Button variant="primary" type="submit" isLoading={isDepositLoading}>Xác nhận</Button>
          </Footer>
        </Form>
      )}
      ...
      {modalStep === 'form' && (
        bankAccounts.length === 0 ? (
          <EmptyBankAccounts>
            <Link2 size={32} />
            <p>Bạn cần liên kết tài khoản ngân hàng trước khi nạp tiền.</p>
            <Button 
              variant="primary" 
              size="sm" 
          ...
          <DepositContent>
            <SelectLabel>Chọn tài khoản ngân hàng nguồn</SelectLabel>
            <select 
